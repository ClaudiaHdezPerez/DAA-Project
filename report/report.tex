...
Dado que se implementaron dos soluciones por fuerza bruta y que, teóricamente, ambas son correctas y tienen la misma complejidad temporal, se decidió realizar una comprobación de manera empírica. Para ello se generaron 100 casos de prueba y ambos algoritmos dieron el mismo resultado en todos los casos, incluyendo casos esquinados. Además, aunque tienen una complejidad similar, los estudios de tiempo determinaron que el segundo algoritmo de fuerza bruta es ligeramente más rápido que el primero (Ver \hyperref[fig:fb_1]{Figura 1} y \hyperref[fig:fb_2]{Figura 2})

\begin{figure}[H]
\centering
\begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_de_tiempos_fuerza_bruta_0.png}
    \caption{Tiempos de ejecución}
    \label{fig:fb_1}
\end{minipage} \hspace{2cm}
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_de_tiempos_fuerza_bruta_1.png}
    \caption{Ratio de aceleración}
    \label{fig:fb_2}
\end{minipage}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{BuyInCurrentPort}: selecciona mercancías prometedoras que generen ganancia al venderlas en el siguiente puerto, ordena descendentemente por la relación ganancia/peso y compra hasta que se incumplan las restricciones de capacidad o capital.
    \item \textbf{SelectNextPortGreedy}: elige el próximo puerto en función de una puntuación heurística


    \[
    \text{score}(p) = \frac{1}{travel\_time(p)} + \left|items(p)\right|
    \]


    donde $travel\_time(p)$ es el tiempo de viaje desde el puerto actual a $p$ y $items(p)$ son los productos disponibles en el puerto $p$ (sin contar los representados como faltantes con infinitos).
    Solo se consideran puertos cuya visita no excede el tiempo máximo permitido. Esta heurística busca favorecer puertos cercanos con más productos para comercializar, que reducen el riesgo de exceder el tiempo máximo y permiten más transacciones.
\end{itemize}
...
\section{Análisis empírico de la solución eficiente}
...
El análisis completo se encuentra en: \\ \url{https://github.com/ClaudiaHdezPerez/DAA-Project/src/validation}. 

\subsection{Calidad de la solución}
La calidad de la solución obtenida por el algoritmo eficiente se comparó frente a la solución óptima de fuerza bruta tanto para variaciones de puertos como variaciones de items utilizando los hiperparámetros de la \hyperref[fig:tab1]{Tabla 1}
...
Se decidió utilizar dos heurísticas para determinar el próximo puerto:
...
\[ H2: \quad
\text{score}(p) = \frac{1}{travel\_time(p)}
\]

$H1$ es la heurística descrita en la sección anterior y $H2$ es una modificación donde solo se consideran las distancias entre los puertos. En cuanto a variaciones de items, ambas tuvieron resultados similares (ver \hyperref[fig:quality_1]{Figura 3} y \hyperref[fig:quality_2]{Figura 4}). 

\begin{figure}[H]
\centering
\begin{minipage}{0.40\textwidth}
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_puertos_fijos_1_0.png}
    \caption{Calidad con puertos fijos considerando distancias y cantidad de productos como heurística}
    \label{fig:quality_1}
\end{minipage}\hspace{2cm}
\begin{minipage}{0.40\textwidth}
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_puertos_fijos_0_0.png}
    \caption{Calidad con puertos fijos solo considerando distancias como heurística}
    \label{fig:quality_2}
\end{minipage}
\end{figure}


Sin embargo, al variar la cantidad de puertos $H1$ se comportó de mejor manera, estando siempre ligeramente más cerca de la solución óptima, además de indicar una posible mejora ante instancias mayores del problema (ver \hyperref[fig:quality_3]{Figura 5} y \hyperref[fig:quality_4]{Figura 6}). Por esta razón se eligió mantener $H1$ como heurística. Después de realizado los estudios se pudo comprobar que la solución eficiente se encuentra, en promedio, por encima del 70\% del óptimo de manera general, y en instancias pequeñas alrededor del 95\%.

\begin{figure}[H]
\centering
\begin{minipage}{0.40\textwidth}
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_items_fijos_1_0.png}
    \caption{Calidad con items fijos considerando distancias y cantidad de productos como heurística}
    \label{fig:quality_3}
\end{minipage}\hspace{2cm}
\begin{minipage}{0.40\textwidth}
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_items_fijos_0_0.png}
    \caption{Calidad con items fijos solo considerando distancias como heurística}
    \label{fig:quality_4}
\end{minipage}
\end{figure}

\subsection{Hiperparámetros de la metaheurística}


Se relizaron estudios sobre la evolución y el rendimiento de la metaheurística empleando diferentes combinaciones de hiperparámetros, teniendo en cuenta los valores de la \hyperref[fig:tab2]{Tabla 2}
...
\hline
\textbf{Parámetro} & \multicolumn{3}{c|}{\textbf{Valores}} \\ \hline
Temperatura inicial & 500 & 1000 & 2000 \\ \hline
Tasa de enfriamiento & 0.98 & 0.995 & 0.999 \\ \hline
Iteraciones máximas & 1000 & 3000 & 5000 \\ \hline
\end{tabular}
\caption{Combinaciones de hiperparámetros estudiadas}
\end{table}


El estudio de las distintas combinaciones determinó que no existe una diferencia significativa de la calidad de la solución entre ellas ni con la utilizada originalmente. Para instancias pequeñas ($n\times m \leq 20$) incluso en tiempo son muy similares aunque en instancias grandes, como se esperaba, al aumentar el número de iteraciones crece el tiempo de ejecución y se nota una mayor diferencia. Todo esto se puede apreciar en la \hyperref[fig:hp]{Figura 7}. Por esas razones, se decidió mantener la combinación de hiperparámetros original.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{./images/analisis_hp_sa_completo.png}
    \caption{Calidad y tiempo de las combinaciones de hiperparámetros estudiadas}
    \label{fig:hp}
\end{figure}
...
Este comportamiento se observa en las comparaciones de ambos algoritmos, en las \hyperref[fig:time_1]{Figura 8} y \hyperref[fig:time_2]{Figura 9}
...
El algoritmo de fuerza bruta puede resolver en tiempos razonables (menos de un minuto ver \hyperref[fig:timeouts_2]{Figura 10}) instancias de 3 puertos y hasta 10 items. A partir de 6 puertos con 4 items, los tiempos crecen de forma exponencial (comportamiento esperado por la complejidad) y superan los 60 segundos (ver \hyperref[fig:timeouts_2]{Figura 11}). 
...
\FOR{each $port$ in $unvisited$}
    ...
    \STATE $travelBack \gets d[port][0]$
    \STATE $totalTime \gets state.time + travelTime + travelBack$
    \IF{$totalTime \leq T_{\max}$}
        \STATE $distanceScore \gets 1 / (travelTime + 10^{-6})$
        \STATE $itemsCount \gets |itemsByPort[port]|$
        \STATE $score \gets distanceScore + itemsCount$
        \IF{$score > bestScore$}
